...
fn run(args: Args) -> Result<()> {
    ...
    let print = |col: Column| {
        let output = match col {
            Col1(val) if args.show_col1 => Some(val.to_string()),
            Col2(val) if args.show_col2 => {
                let padding = if args.show_col1 { &args.delimiter } else { "" };
                Some(format!("{padding}{val}"))
            }
            Col3(val) if args.show_col3 => {
                let padding = match (args.show_col1, args.show_col2) {
                    (true, true) => format!("{d}{d}", d = &args.delimiter),
                    (true, false) | (false, true) => args.delimiter.clone(),
